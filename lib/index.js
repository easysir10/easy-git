 ...
 * ==============================================
 * 标准 DSH 插件（Cordis 插件包）：面向完全不会 git 的用户，提供傻瓜式 git 向导。
 *
 * 提供的工具（注册到 ctx.tools）：
...
export function apply(ctx) {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    '记录用户使用的代码托管平台（GitHub / GitLab / Gitee / 其他），保存后所有 git 引导都会按这个平台定制步骤。用户第一次使用 git、或说“我用 GitHub/GitLab”等话时调用。传 platform 保存；不传则返回当前记录并让用户选择。【务必】把用户当完全不懂 git 的小白：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_status',
    '给完全不懂 git 的用户做“体检”：一次性检查 git 是否安装、当前文件夹是不是 git 仓库、在哪个分支、用户名和邮箱有没有设置、有哪些改动、有没有冲突、是否正在合并中、远程地址、领先/落后远程多少。用户提到 git/提交/拉取/推送/冲突/仓库时，先调用本工具摸清现状，再用大白话把结果讲给用户，并给出明确的下一步建议。【务必】把用户当完全不懂 git 的小白：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
    ...
    async (args, exec) => {
      ...
      const out = []
      let platform = await readPlatform(exec)
      if (!platform && f.remotes.length) platform = inferPlatformFromRemote(f.remotes[0])
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_setup',
    '第一次使用 git 时的初始化配置：设置你的名字和邮箱（写入全局配置，所有仓库都能用）、可选把当前文件夹初始化为 git 仓库、可选绑定 GitLab/GitHub 等平台的仓库地址。执行前先把要做什么用大白话告诉用户并征得同意，再调用本工具。【务必】把用户当完全不懂 git 的小白：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_pull',
    '把代码拉到本地。两种情况：1) 当前文件夹还不是 git 仓库 → 需要用户提供 GitLab/GitHub 等平台的仓库地址（HTTPS 克隆链接），工具会执行克隆；2) 已经是仓库 → 执行拉取（git pull --no-rebase 合并方式，绝不改写用户自己的提交）。出现冲突时提示改用 git_beginner_conflict。执行前向用户解释“拉取=把别人最新改的代码拿下来”。【务必】把用户当完全不懂 git 的小白：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '目标文件夹路径。不填就用当前工作目录。' },
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    '解决“代码冲突”。冲突 = 你和同事改了同一个文件的同一处，git 不知道该保留谁的。先调用本工具查看冲突文件清单；然后向用户用大白话解释选择：①保留我的版本 ②保留对方的版本 ③手动改。用户选好后，把 choice 设为 mine/theirs/manual 再次调用本工具自动解决，或选择 show 查看冲突内容后手动修改（改完用 manual 标记已解决）。建议：构建产物/锁文件/自动生成的文件 → 保留对方的；普通代码两边都改过 → 手动改最稳妥。【务必】把用户当完全不懂 git 的小白：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_commit',
    '把修改保存成一个“版本”（提交）。先调用本工具预览将要提交的文件清单，把清单用大白话告诉用户，并请用户用一句话说明改了什么；拿到那句话后把 message 传进来再次调用。内部执行 git add -A + git commit（提交说明通过标准输入写入，安全无转义问题）。若正在合并中且没有具体文件改动，仍会用提交来完成合并。【务必】把用户当完全不懂 git 的小白：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_push',
    '把本地提交上传到远程仓库（如 GitLab/GitHub）。执行前向用户解释“推送=把你本地的版本上传到服务器，别人就能看到”。首次推送会自动设置上游分支（git push -u origin HEAD）。若被拒绝（远程有别人新提交的代码），提示先调用 git_beginner_pull 合并后再推送；若认证失败，给出个人访问令牌的通俗说明。【务必】把用户当完全不懂 git 的小白：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
    },
    async (args, exec) => {
