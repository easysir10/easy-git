...
export function apply(ctx) {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    async (args, exec) => {
      ...
      else out.push('• 远程仓库：❌ 还没设置（还没连上 GitLab/GitHub）')
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_setup',
    '第一次使用 git 时的初始化配置：设置你的名字和邮箱（写入全局配置，所有仓库都能用）、可选把当前文件夹初始化为 git 仓库、可选绑定 GitLab/GitHub 等平台的仓库地址。执行前先把要做什么用大白话告诉用户并征得同意，再调用本工具。',
    {
    ...
    async (args, exec) => {
      ...
      if (!args.userName && !args.userEmail && !args.initRepo && !args.remoteUrl) {
        return { ok: false, text: 'ℹ️ 这次没有做任何修改。请告诉我：\n• 你的名字（例如：张三）\n• 你的邮箱（例如：zhangsan@example.com，GitLab/GitHub 注册时用的）\n• 是否需要把当前文件夹初始化为 git 仓库\n• 远程仓库地址（可选，从 GitLab/GitHub 项目页 Clone 复制 HTTPS 链接）' }
      }
      out.push('')
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_pull',
    '把代码拉到本地。两种情况：1) 当前文件夹还不是 git 仓库 → 需要用户提供 GitLab/GitHub 等平台的仓库地址（HTTPS 克隆链接），工具会执行克隆；2) 已经是仓库 → 执行拉取（git pull --no-rebase 合并方式，绝不改写用户自己的提交）。出现冲突时提示改用 git_beginner_conflict。执行前向用户解释“拉取=把别人最新改的代码拿下来”。',
    {
      workdir: { type: 'string', description: '目标文件夹路径。不填就用当前工作目录。' },
    ...
    async (args, exec) => {
      ...
      if (!f.isRepo) {
        if (!args.remoteUrl) {
          return { ok: false, text: 'ℹ️ 当前文件夹还不是 git 仓库，需要先有仓库地址才能把代码拉下来。\n\n请去 GitLab/GitHub（或其他代码托管平台）打开项目页面 → 点 Clone 按钮 → 复制 HTTPS 那一行的链接发给我。' }
        }
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_push',
    '把本地提交上传到远程仓库（如 GitLab/GitHub）。执行前向用户解释“推送=把你本地的版本上传到服务器，别人就能看到”。首次推送会自动设置上游分支（git push -u origin HEAD）。若被拒绝（远程有别人新提交的代码），提示先调用 git_beginner_pull 合并后再推送；若认证失败，给出个人访问令牌的通俗说明。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
    },
    async (args, exec) => {
      ...
      if (!f.remotes.length) {
        return { ok: false, text: '❌ 还没绑定远程仓库（GitLab/GitHub 地址）。\n\n去 GitLab/GitHub 新建一个空项目（或直接用已有的），复制它的 HTTPS 克隆链接发给我，我用 git_beginner_setup 帮你绑定。' }
      }
      ...
      if (/Authentication failed|could not read Username|could not read Password|access denied|403|401|Invalid username or password/i.test(text)) {
        return { ok: false, text: '❌ 登录/权限验证没通过。\n\n常见原因和解决办法：\n1. 用 HTTPS 方式：GitLab/GitHub 都不建议用账号密码登录，需要在对应平台里生成“个人访问令牌（Personal Access Token）”，勾选 write_repository 权限；推送时用户名填你的账号、密码填令牌；\n2. 项目是私有的，而你没有该项目权限（找项目管理员加你）；\n3. 也可以改用 SSH 方式（需要配置 SSH 密钥，需要的话我可以一步步教你）。\n\n原始报错：\n' + cap(text, 800) }
      }
