...
export function apply(ctx) {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    '记录用户使用的代码托管平台（GitHub / GitLab / Gitee / 其他），保存后所有 git 引导都会按这个平台定制步骤。用户第一次使用 git、或说“我用 GitHub/GitLab”等话时调用。传 platform 保存；不传则返回当前记录并让用户选择。',
    {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    '解决“代码冲突”。冲突 = 你和同事改了同一个文件的同一处，git 不知道该保留谁的。先调用本工具查看冲突文件清单；然后向用户用大白话解释选择：①保留我的版本 ②保留对方的版本 ③手动改。用户选好后，把 choice 设为 mine/theirs/manual 再次调用本工具自动解决，或选择 show 查看冲突内容后手动修改（改完用 manual 标记已解决）。建议：构建产物/锁文件/自动生成的文件 → 保留对方的；普通代码两边都改过 → 手动改最稳妥。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
