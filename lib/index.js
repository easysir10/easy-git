 ...
 *
 * 提供的工具（注册到 ctx.tools）：
 *   - git_beginner_platform 首次确定平台（GitHub/GitLab/Gitee/其他），之后按平台引导
 ...
 *   - git_beginner_log      大白话查看提交历史
 *   - git_beginner_undo     撤销上一次提交（保留代码，带确认）
 *   - git_beginner_branch   分支管理：新建/切换/合并
 *   - git_beginner_start    “开始使用 git”引导：检查进度并提示下一步
 *
 * 实现要点：
...
export function apply(ctx) {
  ...
  async function collectFacts(workdir, signal) {
    ...
    const hd = await safeGit({ workdir, args: ['rev-parse', '-q', '--verify', 'HEAD'], signal })
    f.hasCommits = hd.exitCode === 0
    const up = await safeGit({ workdir, args: ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}'], signal })
    f.hasUpstream = up.exitCode === 0
    if (fsSvc) {
      try {
        const gi = await fsSvc.resolve('.gitignore', { cwd: workdir || undefined })
        const gs = await fsSvc.stat(gi, signal)
        f.hasGitignore = !!gs
      } catch (e) {
        f.hasGitignore = f.changes.some((c) => c.includes('.gitignore'))
      }
    } else {
      f.hasGitignore = f.changes.some((c) => c.includes('.gitignore'))
    }
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    async (args, exec) => {
      ...
      const ver = await safeGit({ args: ['--version'], signal: exec.signal })
      if (ver.exitCode !== 0) {
        return { ok: false, text: '❌ 这台电脑上还没安装 git，暂时没法记住平台。先安装 git：https://git-scm.com/downloads（一路“下一步”即可），装好后再来。' }
      }
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    async (args, exec) => {
      ...
      if (args.remoteUrl) {
        ...
        } else if (!/^(https?:\/\/|git@|ssh:\/\/)/i.test(args.remoteUrl)) {
          ok = false
          out.push('❌ 这个地址看起来不像仓库地址（应以 https:// 或 git@ 开头，例如 https://github.com/xxx/yyy.git）。请重新复制项目页的克隆链接。')
        } else {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    async (args, exec) => {
      ...
      if (!f.isRepo) {
        ...
        const plat = PLATFORM_INFO[await readPlatform(exec)] || null
        return { ok: false, text: '❌ 克隆失败。\n\n' + (r.timedOut ? '超时了，可能是网络太慢，稍后再试一次。' : '常见原因：\n1. 地址写错了（注意要以 https:// 开头）\n2. 项目是私有的，需要先登录/授权（' + (plat ? '去' + plat.name + '生成“个人访问令牌”：' + plat.tokenHint + '，克隆时用户名填账号、密码填令牌' : 'GitLab/GitHub 都可以用“个人访问令牌”当密码') + '）\n3. 网络不通也会导致失败，可以稍后再试\n\n【技术报错，助手自己看，不用给用户看】\n' + raw) }
      }
      if (f.conflicts.length) {
      ...
      if (!f.remotes.length && args.remoteUrl) {
        if (!/^(https?:\/\/|git@|ssh:\/\/)/i.test(args.remoteUrl)) {
          return { ok: false, text: '❌ 这个地址看起来不像仓库地址（应以 https:// 或 git@ 开头）。请重新复制项目页的克隆链接。' }
        }
        const add = await safeGit({ workdir, args: ['remote', 'add', 'origin', args.remoteUrl], signal: exec.signal })
      ...
      if (!f.hasUpstream && !args.branch) {
        return { ok: false, text: 'ℹ️ 当前分支（' + f.branch + '）还没和远程建立连接，git 不知道该拉哪个分支。\n\n两种办法（选一个告诉我）：\n1️⃣ 先“推送”一次（会自动建立连接，推荐）\n2️⃣ 告诉我远程分支名，我用 git_beginner_pull 的 branch 参数帮你指定' }
      }
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    async (args, exec) => {
      const workdir = args.workdir || await resolveDefaultWorkdir(exec)
      const f = await collectFacts(workdir, exec.signal)
      if (!f.gitInstalled) return { ok: false, text: '❌ 这台电脑上还没安装 git。先安装：https://git-scm.com/downloads（一路“下一步”即可），装好后再试。' }
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    async (args, exec) => {
      ...
      if (!args.message) {
        ...
        if (!f.hasGitignore && f.changes.some((c) => c.startsWith('??'))) {
          out.push('📌 提示：你的仓库还没有 .gitignore 文件。如果这次要提交的文件里有不该入库的（比如 node_modules、target、.idea、编译产物），告诉我，我可以帮你建一个 .gitignore 自动排除它们。')
        }
        out.push('')
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    ...
    async (args, exec) => {
      ...
      if (!f.isRepo) return { ok: false, text: '当前文件夹还不是 git 仓库，先拉取或初始化再说。' }
      if (!f.hasCommits) {
        return { ok: false, text: 'ℹ️ 你还没有提交过任何代码，所以没有东西可以推送。\n\n先告诉我“提交”，把代码保存成第一个“快照”，再推送。' }
      }
      if (f.conflicts.length) {
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_log',
    '大白话查看提交历史：列出最近的提交（时间、作者、说明）。用户问“看看我提交过什么 / 历史记录 / 日志”时调用。不填 count 默认最近 10 条。【务必】把用户当成第一次接触 git 的新手：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
      count: { type: 'integer', description: '要看最近几条（默认 10，最多 50）。' },
    },
    async (args, exec) => {
      const workdir = args.workdir || await resolveDefaultWorkdir(exec)
      const f = await collectFacts(workdir, exec.signal)
      if (!f.gitInstalled) return { ok: false, text: '❌ 这台电脑上还没安装 git。先安装：https://git-scm.com/downloads（一路“下一步”即可），装好后再试。' }
      if (!f.isRepo) return { ok: false, text: '当前文件夹还不是 git 仓库，先初始化或拉取再说。' }
      if (!f.hasCommits) return { ok: true, text: '📜 这个仓库还没有任何提交。先“提交”一次，就有了第一条历史。' }
      const n = Math.max(1, Math.min(args.count || 10, 50))
      const log = await safeGit({ workdir, args: ['log', '-n', String(n), '--pretty=format:%h%x1f%ad%x1f%an%x1f%s', '--date=format:%Y-%m-%d %H:%M'], stdoutMaxBytes: 200000, signal: exec.signal })
      if (log.exitCode !== 0) return { ok: false, text: '❌ 查看历史失败：\n【技术报错，助手自己看，不用给用户看】\n' + cap(log.stderr, 500) }
      const lines = cleanLines(log.stdout)
      if (!lines.length) return { ok: true, text: '📜 这个仓库还没有任何提交。' }
      const out = []
      out.push('📜 最近 ' + lines.length + ' 次提交（最上面是最新）：')
      out.push('')
      for (const line of lines) {
        const p = line.split('\x1f')
        if (p.length >= 4) out.push('• ' + p[1] + '　' + p[3] + '（' + p[2] + '）')
        else out.push('• ' + line)
      }
      out.push('')
      out.push('想看某一次提交改了哪些文件，告诉我它的时间或说明里的关键词，我来打开。')
      return { ok: true, text: out.join('\n') }
    },
  )))
  ...
  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_undo',
    '撤销上一次提交（后悔药）：撤回最近一张“快照”，但代码改动全部保留，可以重新提交。第一次调用先说明后果并请用户确认；用户确认后（confirm: true）再执行。已推送过的提交不建议撤销。【务必】把用户当成第一次接触 git 的新手：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
      confirm: { type: 'boolean', description: '用户确认撤销后再传 true 执行。第一次调用不填，先预览说明。' },
    },
    async (args, exec) => {
      const workdir = args.workdir || await resolveDefaultWorkdir(exec)
      const f = await collectFacts(workdir, exec.signal)
      if (!f.gitInstalled) return { ok: false, text: '❌ 这台电脑上还没安装 git。先安装：https://git-scm.com/downloads（一路“下一步”即可），装好后再试。' }
      if (!f.isRepo) return { ok: false, text: '当前文件夹还不是 git 仓库。' }
      if (f.merging) return { ok: false, text: '⚠️ 当前正在合并中，不能撤销。先告诉我“提交”把合并完成，再来撤销。' }
      if (f.conflicts.length) return { ok: false, text: '⚠️ 还有冲突没解决，先解决冲突再说。' }
      if (!f.hasCommits) return { ok: true, text: 'ℹ️ 还没有任何提交可以撤销。' }
      const cnt = await safeGit({ workdir, args: ['rev-list', '--count', 'HEAD'], signal: exec.signal })
      const total = cnt.exitCode === 0 ? parseInt(cnt.stdout.trim(), 10) : 0
      if (total <= 1) {
        return { ok: false, text: 'ℹ️ 这是你的第一个提交，没有更早的可以撤销。\n\n如果你想“重新开始”，告诉我，我可以帮你把历史清空重新提交（这个操作有风险，我会先跟你确认再动）。' }
      }
      if (!args.confirm) {
        let text = '⚠️ 撤销上一次提交 = 把最近这张“快照”撤掉，**你改的代码一个都不会丢**（只是回到提交前的状态，改动都还在）。'
        if (f.hasUpstream) text += '\n\n⚠️ 你的仓库连着远程：如果上次提交已经推送上去了，撤销后本地会“落后”远程，再推送会变麻烦——**先确认这次提交还没传上去，再撤销**。'
        text += '\n\n确认要撤销吗？确认的话告诉我“确认撤销”，我再执行。'
        return { ok: true, text }
      }
      const r = await safeGit({ workdir, args: ['reset', '--soft', 'HEAD~1'], signal: exec.signal })
      if (r.exitCode !== 0) return { ok: false, text: '❌ 撤销失败：\n【技术报错，助手自己看，不用给用户看】\n' + cap(r.stderr, 500) }
      const last = await safeGit({ workdir, args: ['log', '-1', '--pretty=format:%s'], signal: exec.signal })
      return { ok: true, text: '✅ 已撤销上一次提交（“' + (last.exitCode === 0 ? last.stdout.trim() : '') + '”）。\n\n你的代码改动都还在，现在处于“提交前”状态。可以继续改，改好了再“提交”。' }
    },
  )))

  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_branch',
    '分支管理。分支 = 代码的平行线，在一条线上改，不影响主代码（main）。支持 list（查看）/ create（新建并切过去）/ switch（切换）/ merge（把另一个分支合并进当前）。用户说“我想开个新分支 / 切换分支 / 合并分支”时调用。【务必】把用户当成第一次接触 git 的新手：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
      action: { type: 'string', enum: ['list', 'create', 'switch', 'merge'], description: 'list=查看分支（默认）；create=新建并切换（需 name）；switch=切换（需 name）；merge=把 name 分支合并进当前分支。' },
      name: { type: 'string', description: '分支名（create/switch/merge 时必填）。' },
    },
    async (args, exec) => {
      const workdir = args.workdir || await resolveDefaultWorkdir(exec)
      const f = await collectFacts(workdir, exec.signal)
      if (!f.gitInstalled) return { ok: false, text: '❌ 这台电脑上还没安装 git。先安装：https://git-scm.com/downloads（一路“下一步”即可），装好后再试。' }
      if (!f.isRepo) return { ok: false, text: '当前文件夹还不是 git 仓库，先初始化或拉取再说。' }
      if (f.merging || f.conflicts.length) return { ok: false, text: '⚠️ 当前有冲突或合并还没完成，先处理完再动分支。' }
      const action = args.action || 'list'
      if (action === 'list') {
        const br = await safeGit({ workdir, args: ['branch', '-a'], stdoutMaxBytes: 200000, signal: exec.signal })
        const out = []
        out.push('🌿 当前分支：' + f.branch)
        out.push('')
        out.push('分支 = 代码的平行线，在一条线上改，不影响主代码（main）。')
        out.push('')
        if (br.exitCode === 0 && br.stdout.trim()) {
          out.push('所有分支（带 * 的是你现在的）：')
          for (const line of br.stdout.split(/\r?\n/)) {
            const t = line.trim()
            if (t) out.push('  ' + t)
          }
        } else {
          out.push('（目前只有 main 一个分支）')
        }
        out.push('')
        out.push('想新建分支就说“开个新分支叫 xxx”，想切换就说“切到 xxx”，想合并就说“把 xxx 合并进来”。')
        return { ok: true, text: out.join('\n') }
      }
      if (!args.name) return { ok: false, text: '需要告诉我分支名（比如：开个新分支叫 feature-1）。' }
      if (action === 'create') {
        const chk = await safeGit({ workdir, args: ['checkout', '-b', args.name], signal: exec.signal })
        if (chk.exitCode === 0) {
          return { ok: true, text: '✅ 已新建分支“' + args.name + '”并切过去了（从当前代码出发）。\n\n你现在在' + args.name + '分支上，随便改，不影响 main。改完提交、推送，之后可以再合并回 main。' }
        }
        if (/already exists/i.test(chk.stderr)) return { ok: false, text: '❌ 叫“' + args.name + '”的分支已经存在了。换一个名字，或者告诉我“切到 ' + args.name + '”。' }
        return { ok: false, text: '❌ 新建分支失败：\n【技术报错，助手自己看，不用给用户看】\n' + cap(chk.stderr, 500) }
      }
      if (action === 'switch') {
        const chk = await safeGit({ workdir, args: ['checkout', args.name], signal: exec.signal })
        if (chk.exitCode === 0) return { ok: true, text: '✅ 已切换到分支“' + args.name + '”。' }
        if (/did not match|no such branch|paths/i.test(chk.stderr)) {
          return { ok: false, text: '❌ 找不到叫“' + args.name + '”的分支。可以先“开个新分支叫 ' + args.name + '”，或让我看看有哪些分支。' }
        }
        if (/local changes|overwritten|overwrite/i.test(chk.stderr)) {
          return { ok: false, text: '⚠️ 你还有没提交的修改，git 怕弄丢不敢切换。先“提交”把修改保存起来，再切换。' }
        }
        return { ok: false, text: '❌ 切换失败：\n【技术报错，助手自己看，不用给用户看】\n' + cap(chk.stderr, 500) }
      }
      if (action === 'merge') {
        const mg = await safeGit({ workdir, args: ['merge', '--no-edit', args.name], timeoutMs: 120000, signal: exec.signal })
        const text = (mg.stdout || '') + (mg.stderr || '')
        if (mg.exitCode === 0) return { ok: true, text: '✅ 已把“' + args.name + '”的改动合并进当前分支（' + f.branch + '）。\n\n' + cap(text, 500) }
        if (/CONFLICT|Automatic merge failed|conflict/i.test(text)) {
          return { ok: false, text: '⚠️ 合并时出现了冲突（两边改了同一处）。别担心，这很正常。请用 git_beginner_conflict 工具，我带你一步步选。' }
        }
        return { ok: false, text: '❌ 合并失败：\n【技术报错，助手自己看，不用给用户看】\n' + cap(text, 800) }
      }
      return { ok: false, text: '未知的操作类型。' }
    },
  )))

  disposers.push(ctx.tools.register(defineToolFor(
    'git_beginner_start',
    '“开始使用 git”引导：检查配置进度（平台 → 身份 → 仓库），告诉用户下一步该做什么、调用哪个工具完成。用户第一次说“开始用 git / 我要怎么开始 / 我是新手”时调用。【务必】把用户当成第一次接触 git 的新手：全程中文大白话，绝不让用户输入任何命令行命令，绝不让用户填文件夹路径（用默认工作区即可），专业词出现时先一句话解释。',
    {
      workdir: { type: 'string', description: '仓库文件夹路径。不填就用当前工作目录。' },
    },
    async (args, exec) => {
      const workdir = args.workdir || await resolveDefaultWorkdir(exec)
      const f = await collectFacts(workdir, exec.signal)
      const plat = await readPlatform(exec)
      const done = []
      const todo = []
      if (plat) done.push('✅ ① 平台：你用的是' + PLATFORM_INFO[plat].name)
      else todo.push('① 平台：还没选。问用户用哪个平台（GitHub / GitLab / Gitee / 其他），选好后调用 git_beginner_platform 保存')
      if (f.gitInstalled && f.userName && f.userEmail) done.push('✅ ② 身份：名字（' + f.userName + '）和邮箱已设置')
      else if (f.gitInstalled) todo.push('② 身份：名字/邮箱还没设置。问用户名字和邮箱，调用 git_beginner_setup')
      else todo.push('② 身份：git 还没安装（先让用户装好 git 再继续，地址 https://git-scm.com/downloads）')
      if (f.isRepo) done.push('✅ ③ 仓库：当前文件夹已经是 git 仓库')
      else todo.push('③ 仓库：还没有。问用户：a) 从远程拉一个项目（调用 git_beginner_pull，需要仓库地址）还是 b) 本地初始化（调用 git_beginner_setup 的 initRepo）')
      const out = []
      out.push('🎯 开始使用 git —— 进度报告：')
      out.push('')
      for (const d of done) out.push(d)
      for (const t of todo) out.push('⬜ ' + t)
      out.push('')
      if (!todo.length) {
        out.push('🎉 配置全齐了！现在就可以开始：改代码后说“提交”，上传说“推送”，同步说“拉取”。')
      } else {
        out.push('下一步：先完成最上面那个 ⬜ 步骤（我会一步步带你，每步都会先跟你确认）。')
      }
      return { ok: true, text: out.join('\n') }
    },
  )))
